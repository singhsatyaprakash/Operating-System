...
int main() {
    ...
    while (completed != n) {
        // Find process with highest priority in ready queue at currTime
        int index = -1;
        int priority = INT_MIN;
        for (int i = 0; i < n; i++) {
            if (ps[i].at <= currTime && !is_completed[i] && bt_remaining[i] > 0) {
                if (ps[i].pt > priority) {
                    priority = ps[i].pt;
                    index = i;
                }
        ...
        if (index == -1) {
            ...
            if (bt_remaining[index] == ps[index].bt) {//reaches first time so set startime....
                ps[index].start_time = currTime;
                total_idle_time += is_first_process ? 0 : (ps[index].start_time - prev);
